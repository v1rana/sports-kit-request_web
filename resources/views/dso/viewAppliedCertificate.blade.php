...
.btn-area{width:100%; max-width: 800px;margin:0 auto;}
    ...
    <button onclick="window.print()" class="print-button btn btn-success float-end">Print Certificate</button>
        ...
        <p>Application No: {{ $otpData->appl_id }}</p>
        ...
        <div class="content">
            ...
            <ul class="achievement-details">
                <li style="padding: 5px;display: block;">i). Name of Tournament: <strong class="underline">{{ $otpData->tournament ?? '' }}<!-- {{ $otpData->tournament_name ?? '' }} --></strong></li>
                <li style="padding: 5px;display: block;">ii). Date of Event: <strong class="underline">{{ $otpData->month_year ?? '' }}<!-- {{ $otpData->month_year ?? '' }} --></strong></li>
            ...
            <table class="signature-date-area">
                <tr>
                    <td>Date of form submission <strong class="underline">{{ $otpData->created_at ?? '' }}<!-- {{ $otpData->created_at ?? '' }} --></strong></td>
			...
			<table width="100%">
                <tr>
                   <td> Date of Issuance: <strong class="underline">{{ \Carbon\Carbon::now()->format('d-m-Y') }}</strong></td>
                    <td style="text-align:right">Granted Grade <strong class="underline" style="width: 50px; text-align:center; padding:0">{{ $otpData->gradation ?? '' }}</strong> Sports Certificate</td>
                </tr>
            </table>
        </div>
