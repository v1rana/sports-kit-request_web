            ...
            <tbody>
                @foreach ($sportsCertificates as $index => $certificate)
                    <tr>
                        ...
                        <td>
                            <div class="d-flex">
                                <button type="button" class="bg-transparent text-primary app-id-view-btn"
                                    data-bs-toggle="modal"
                                    data-bs-target="#modal{{ $certificate->id }}">{{ $certificate->appl_id }}</button>
                                @if ($certificate->verification_by_sportsperson)
                                    <a href="{{ url('storage/' . $certificate->verification_by_sportsperson) }}"
                            ...
                            <div class="modal fade" id="modal{{ $certificate->id }}" data-bs-backdrop="static"
                                data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel"
                                aria-hidden="true">
                                <div class="modal-dialog modal-dialog-centered modal-xl">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title float-start" id="modal{{ $certificate->id }}">Gradation
                                                Application ID - <span>{{ $certificate->appl_id }}</span></h5>
                                            <h5 class="modal-title float-end" id="modal{{ $certificate->id }}">Certificate
                                                Grade - <span>{{ $certificate->gradation }}</span> </h5>
                                        ...
                                        <div class="modal-body">
                                            <div class="d-flex justify-content-between mb-3">
                                                <div class="">
                                                    ...
                                                    <h5> {{ \Carbon\Carbon::parse($certificate->date)->format('d M Y') }}
                                                    </h5>
                                                </div>
                                                <div class="co">
                                                ...
                                                <div class="">
                                                    ...
                                                    <h6>
                                                        <strong>
                                                            <div id="approval-section-{{ $certificate->id }}"
                                                                style="{{ !empty($certificate->enquiry_pdf) && !empty($certificate->replied_pdf) && $certificate->status != 'Approved' && $certificate->status != 'Rejected' ? '' : 'display:none;' }}">
                                                                <form action="{{ route('dso.approve', $certificate->id) }}"
                                            ...
                                            <h3 class="modal-title-details"><i class="fa-solid fa-user-large"></i> Personal
                                                Details</h3>
                                            <div class="border p-3">
                                                <div class="row">
                                                    <div class="col-xs-12 col-sm-12 col-md-10">
                                                        <div class="row">
                                                            <div class="col-md-3 mb-3">
                                                                <small class="info-label text-muted ">1. Sports person's
                                                                    Name</small>
                                                            ...
                                                            <div class="col-md-3 mb-3">
                                                                <small class="info-label text-muted ">4. Name of District
                                                                    sportsperson belongs to</small>
                                                                <h5>{{ $certificate->district_sportsperson_belongs }}</h5>
                                                            </div>
                                                            <div class="col-md-3 mb-3">
                                                                <small class="info-label text-muted ">5. Domicile
                                                                    State</small>
                                                                <h5>{{ $certificate->domicile_state }}</h5>
                                                            </div>
                                                            <div class="col-md-3 mb-3">
                                                                <small class="info-label text-muted ">6. Plays for (Name of
                                                                    State/ Organization)</small>
                                                                <h5>{{ $certificate->plays_for_statte_org }}</h5>
                                                            </div>
                                                            <div class="col-md-3 mb-3">
                                                                <small class="info-label text-muted ">7. Name of Sports
                                                                    Discipline</small>
                                                                <h5>{{ $certificate->name_sports_discipline }}</h5>
                                                            </div>
                                                            <div class="col-md-3 mb-3">
                                                                <small class="info-label text-muted ">8. Type of
                                                                    Event</small>
                                                    ...
                                                    <div class="col-xs-12 col-sm-12 col-md-2">
                                                        <img src="{{ asset('storage/' . $certificate->profile_picture) }}"
                                                            width="150px" height="150px" style="border:5px solid #eee">
                                            ...
                                            <div class="border p-3">
                                                <div class="row">
                                                    ...
                                                    <div class="col-xs-12 col-sm-6 col-md-2 mb-3">
                                                        ...
                                                        <h5>{{ \Carbon\Carbon::createFromFormat('Y-m-d', $certificate->month_year)->format('F Y') }}
                                                        </h5>
                                                    </div>
                                                    <div class="col-xs-12 col-sm-6 col-md-2 mb-3">
                                                    ...
                                                    <div class="col-xs-12 col-sm-6 col-md-8 mb-3">
                                                        <small class="info-label text-muted ">4. Organising
                                                            Authority</small>
                                       ...
                                       <h6>{{ $certificate->gender }}</h6>
                                                    ...
                                                    <div class="col-xs-12 col-sm-6 col-md-2 mb-3">
                                                        <small class="info-label text-muted ">6. Participation
                                                            Level</small>
                                            ...
                                            <h3 class="modal-title-details"><i class="fa-solid fa-folder-open"></i>
                                                Application Enquiry</h3>
                                            <div class="border p-3">
                                                <div class="row">
                                                            ...
                                                            <div>
                                                                ...
                                                                <p>
                                                                    <small class="mb-0 text-muted text-end">
                                                                        Date:
                                                                        {{ \Carbon\Carbon::parse($certificate->enquiry_pdf_datetime)->format('d M Y') }}
                                                                    </small>
                                                                </p>
                                                            </div>
                                                        @else
                                                            <form id="enquiry-upload-form-{{ $certificate->id }}"
                                                                action="{{ route('dso.enquiry.UploadLetter') }}"
                                                                method="POST" enctype="multipart/form-data">
                                                                @csrf
                                                                <input type="file" name="enquiry_pdf"
                                                            ...
                                                            <div>
                                                                <a href="{{ asset('storage/' . $certificate->replied_pdf) }}"
                                                                    target="_blank" class="btn btn-primary mb-2"> <i
                                                                        class="fa-solid fa-file-lines"></i> View Reply
                                                                    Letter </a>

                                                                <p><small class="mb-0 text-muted text-end">
                                                                        Date:
                                                                        {{ \Carbon\Carbon::parse($certificate->replied_pdf_datetime)->format('d M Y') }}
                                                                    </small></p>
                                                            </div>
                                                        @else
                                                            <form id="reply-upload-form-{{ $certificate->id }}"
                                                                action="{{ route('dso.enquiry.ReplyLetter') }}"
                                                                method="POST" enctype="multipart/form-data">
                                                                @csrf
                                                                <input type="file" name="replied_pdf"
                                                                    accept="application/pdf" required
                                                                    class="form-control">
                                                                <input type="hidden" name="certificate_id"
                                                                    value="{{ $certificate->id }}">
                                                                <button type="submit" class="btn btn-warning mt-2"
                                                                    style="margin-left:5px">Upload</button>
                                                            </form>
                                                            <div id="reply-upload-status-{{ $certificate->id }}"
                                                                class="mt-2"></div>
                                                        @endif
                                                    </div>
                                                    ...
                                                    <div class="col-xs-12 col-sm-6 col-md-3">
                                                        @if (!empty($certificate->enquiry_pdf) && !empty($certificate->replied_pdf) && $certificate->status == 'Approved')
                                                            <label class="info-label text-muted">Download
                                                                Certificate</label>
                                                            <form id="pdfForm{{ $certificate->id }}" method="POST"
                                                                action="{{ route('dso.certificates.downloadPDF') }}"
                                                                target="_blank">
                                                                @csrf
                                                                <input type="hidden" name="certificate_id"
                                                                    value="{{ $certificate->id }}">
                                                        ...
                                                        @if (!empty($certificate->enquiry_pdf) && !empty($certificate->replied_pdf) && $certificate->status == 'Approved')
                                                            @if ($certificate->certificate_pdf)
                                                                ...
                                                                @else
                                                                    ...
                                                                    <form
                                                                        action="{{ route('dso.certificates.uploadPDF') }}"
                                                                        method="POST" enctype="multipart/form-data"
                                                ...
                                                @php
                                                    ...
                                                    $endDate = $certificate->certificate_upload_datetime
                                                        ? \Illuminate\Support\Carbon::parse(
                                                            $certificate->certificate_upload_datetime,
                                                        )
                                                        : now();
                        ...
                        <td>
                            <strong>
                                @if ($certificate->status == 'Approved' && empty($certificate->certificate_pdf))
                                    <span class="badge rounded-pill bg-success"><i class="fa-solid fa-thumbs-up"></i>
                                        Approved</span>
                                @elseif($certificate->status == 'Approved' && !empty($certificate->certificate_pdf))
                                    <span class="badge rounded-pill bg-success"><i class="fa-solid fa-thumbs-up"></i>
                                    ...
                                    <a href="{{ asset('storage/' . $certificate->certificate_pdf) }}" target="_blank"
                                        style="text-decoration: underline; color: #0d6efd;font-size: 9px;"
                                        class="mt-2 d-inline-block">
                                        View Issued Certificate
                                    </a>
                                @elseif($certificate->status == 'Rejected')
                                    <span class="badge rounded-pill bg-danger"><i class="fa-solid fa-ban"></i>
                                        Rejected</span>
                                @else
                                    <span class="badge rounded-pill bg-primary w-100"><i class="fa-solid fa-check"></i>
                                        In-Progress</span>
                            ...
                            <button type="button" class="btn btn-danger w-100" data-bs-toggle="modal" data-bs-target="#rejectRemarkModal{{ $certificate->id }}">
                              Reject
                                    ...
                                    <!-- Modal -->
                                    <div class="modal fade" id="rejectRemarkModal{{ $certificate->id }}" tabindex="-1"
                                        aria-labelledby="rejectRemarkModalLabel" aria-hidden="true">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            replyForms.forEach(form => {
                form.addEventListener('submit', function(e) {
                    ...
                    const approvalDiv = document.getElementById(
                        `approval-section-${certId}`
                    ); // jo div tumne blade me add kiya, initially hidden
                    const submitBtn = form.querySelector('button[type="submit"]');
                    ...
                    `;
                                ...
                                statusDiv.innerHTML =
                                    `<p class="text-danger">Upload failed: ${data.message}</p>`;
                            }
                        })
                        .catch(error => {
